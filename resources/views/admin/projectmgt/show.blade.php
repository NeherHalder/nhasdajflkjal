...
<div class="row">
	@if($project->status == 0)
	<div id="lottery_btn" class="col-md-6">
	...
	<div id="show" style="display: none;" class="col-md-4">
		<form class="form-inline" method="post" action="{{ url(route('lottery.create')) }}">
			{{ csrf_field() }}
		  	<div class="form-group">
		    	<input type="number" class="form-control" name="limit" placeholder="Set Limit" required>
		    	<input type="hidden" name="id" value="{{ $project->id }}">
		  	</div>
			<button type="submit" class="btn btn-warning" onclick="return comfirm()">Start</button>
		</form>
	</div>
	@else
	<div class="col-md-6">
		<a href="{{url(route('lottery.winners', [$project->id]))}}" class="btn btn-block btn-warning btn-lg"
			style="height: 100px; line-height: 80px;text-align: center;font-size: 1.5em;"
		>
			<i class="material-icons">gamepad</i>
			WINNERS LIST
...
<script type="text/javascript">
	...
	function comfirm(){
		return confirm('Are you sure want to start lottery?');
	}
</script>
<script type="text/javascript">
